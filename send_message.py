...
# 用户 OpenID 列表
openids = [
    "o603S6X7MkTBvEzPpWm-gDZQn-V8",  # 用户1的 OpenID
    "o603S6T-2iph72iNREYCAAvoMBnE"   # 用户2的 OpenID
]
...
def get_weather(city_code, api_key):
    ...
    data = response.json()
    print("Weather API Response:", data)  # 打印 API 响应数据
...
def send_message():
    ...
    message_data_template = {
        ...
        "data": {
            ...
            "girlfriend_birthday_days_left": {"value": f"{girlfriend_birthday_days_left}", "color": "#FF1493"},
    ...
    for openid in openids:
        message_data = message_data_template.copy()
        message_data["touser"] = openid  # 设置当前用户的 OpenID

        url = f"https://api.weixin.qq.com/cgi-bin/message/template/send?access_token={access_token}"
        response = requests.post(url, json=message_data)
        print(f"Response for {openid}:", response.text)  # 打印返回的响应内容
